...
### Cloud Scheduler ジョブ（統合版・2026年1月22日更新）

| 種類 | ジョブ名 | スケジュール | 説明 |
|---|---|---|---|
| 一般予約（午前） | `timeslot-checker-unified-general-morning` | `*/1 7-12 * * 0-2,4-6` | 7:00-12:59、1分毎（水曜除く） |
| 一般予約（午後） | `timeslot-checker-unified-general-afternoon` | `*/2 13-16 * * 0-2,4-6` | 13:00-16:59、2分毎（水曜除く） |
...
**概要:**
一般予約と視野予約で別々に稼働していた2つのCloud Runサービスを、1つの統合版サービスに統合しました。これにより、Cold startコストが半減し、月額コストを削減しました。
...
- URLパラメータ `?type=general` / `?type=shiya` で処理を分岐
- Cloud Storageへの出力ファイルは従来と同一（既存システムへの影響なし）
- チェック頻度（一般1分/視野3分等）はCloud Scheduler側で制御（変更なし）
...
- タイルのセルが正しく色分けされない
- 2時間以内に埋まった時の斜線パターンが表示されない
- キャンセル発生時の点線枠が表示されない
- ※折れ線グラフは正常に動作

#### 🔍 原因

**履歴データに`slots`フィールドが含まれていなかった**

調査結果：
- **1月10日のデータ**: `slots`あり ✅
- **1月11日以降のデータ**: `slots`なし ❌
...
- [ ] 現在の履歴データに`slots`フィールドが含まれていることを確認
  ```bash
...
- [ ] **必ずCloud Runをリデプロイする**（キャッシュ無効化付き）
  ```bash
...
- [ ] 数分待ってから履歴データを確認
  ...
  ```
- [ ] **`"slots": [...]`** が含まれていることを確認
- [ ] ダッシュボードの「時間枠別 埋まり推移」が正常に表示されることを確認

#### ❌ よくある失敗パターン

1. **Cloud Schedulerだけ変更してCloud Runをリデプロイしない**
   → 古いコードが動作し続け、`slots`が保存されない

2. **リデプロイしたがキャッシュ無効化を忘れた**
   → Cloud Buildがキャッシュを使い、古いコードがデプロイされる

3. **確認せずに作業完了とした**
   → 数日後にダッシュボードが壊れていることに気づく
