...
function updateBadge(isAvailable) {
  if (isAvailable) {
    // 空きあり: アイコンは緑（Canvas APIで動的生成されるため、ここでは何もしない）
    chrome.action.setTitle({ title: '予約状況（一般）: 空きあり' });
    console.log('[一般予約] バッジ更新: 空きあり（緑）');
  } else {
    // 満枠: アイコンは赤+白バツ
    chrome.action.setTitle({ title: '予約状況（一般）: 満枠' });
    console.log('[一般予約] バッジ更新: 満枠（赤+白バツ）');
  }
...
function updateBadgeError() {
  chrome.action.setTitle({ title: '予約状況（一般）: エラー' });
