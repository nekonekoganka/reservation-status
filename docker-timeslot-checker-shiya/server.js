/**
 * 視野予約時間枠表示システム（Docker版）
 *
 * 視野予約ページから具体的な空き時間枠を取得し、Cloud Storageに保存
...
const FILE_NAME = 'timeslots-shiya.json';
const HISTORY_PREFIX = 'history/shiya'; // 履歴保存先
...
app.get('/generate-daily-summary', async (req, res) => {
  ...
  try {
    ...
    // Cloud Storageに保存
    const summaryFileName = `daily-summary/shiya/${targetDateStr}.json`;
    ...
    console.log('=== 日次サマリー生成完了（視野） ===');

    res.status(200).json({
      success: true,
      message: '日次サマリーを生成しました（視野）',
      date: targetDateStr,
...
app.get('/generate-all-daily-summaries', async (req, res) => {
  ...
  try {
    ...
    console.log('=== 全日次サマリー一括生成完了（視野） ===');

    res.status(200).json({
      success: true,
      message: `${daysToGenerate}日分の日次サマリーを生成しました（視野）`,
      results
    });
...
app.get('/generate-monthly-summary', async (req, res) => {
  ...
  try {
    ...
    // Cloud Storageに保存
    const summaryFileName = `monthly-summary/shiya/${monthStr}.json`;
    const summaryFile = bucket.file(summaryFileName);
    ...
    console.log('=== 月次集計データ生成完了（視野） ===');

    res.status(200).json({
      success: true,
      message: '月次集計データを生成しました（視野）',
      month: monthStr,
      daysWithData: Object.keys(dailyData).length,
      totalDataPoints,
      publicUrl: `https://storage.googleapis.com/${BUCKET_NAME}/${summaryFileName}`
...
app.get('/generate-all-monthly-summaries', async (req, res) => {
  ...
  try {
    ...
    console.log('=== 全月次集計データ一括生成完了（視野） ===');

    res.status(200).json({
      success: true,
      message: `${monthsToGenerate}ヶ月分の月次集計を生成しました（視野）`,
      results
    });
...
app.listen(PORT, () => {
  console.log('==============================================');
  console.log('視野予約時間枠表示システム 起動しました');
