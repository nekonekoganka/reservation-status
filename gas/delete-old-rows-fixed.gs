/**
 * Google Apps Script - スプレッドシートに書き込むWeb App（統合版）
 *
 * このファイルは、現在Google Apps Scriptエディタで使用中のコードのバックアップです。
 * 以下の機能が統合されています：
 * 1. Chrome拡張機能/Docker自動化からのデータ書き込み（doPost）
 * 2. 古い行の自動削除（deleteOldRows）- 週1回自動実行
 */
 ...
 *
 * 注意：SPREADSHEET_IDとSHEET_NAMEは上記で定義された定数を使用します
 */
