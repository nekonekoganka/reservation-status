// 視野予約枠数表示 - Background Service Worker

const TIMESLOTS_URL = 'https://storage.googleapis.com/reservation-timeslots-fujiminohikari/timeslots-shiya.json';
const UPDATE_INTERVAL = 1; // 分

// インストール時・起動時の初期化
chrome.runtime.onInstalled.addListener(() => {
  console.log('[視野予約枠数] 拡張機能がインストールされました');
  updateStatus();
  setupAlarm();
});

chrome.runtime.onStartup.addListener(() => {
  console.log('[視野予約枠数] 拡張機能が起動しました');
  updateStatus();
  setupAlarm();
});
...
function setupAlarm() {
  ...
  console.log(`[視野予約枠数] アラーム設定完了: ${UPDATE_INTERVAL}分ごとに更新`);
}

// アラームのリスナー
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === 'updateStatus') {
    console.log('[視野予約枠数] アラーム発火 - ステータス更新開始');
    updateStatus();
  }
});
...
async function updateStatus() {
  try {
    console.log('[視野予約枠数] ステータス更新を開始します...');

    // Cloud Storageからデータ取得
    const data = await fetchTimeslotsData();

    if (!data) {
      console.error('[視野予約枠数] データ取得に失敗しました');
      updateBadgeError();
      return;
    }

    console.log('[視野予約枠数] データ取得成功:', data);
    ...
    console.log('[視野予約枠数] ステータス更新完了');

  } catch (error) {
    console.error('[視野予約枠数] ステータス更新エラー:', error);
    updateBadgeError();
  }
}
...
async function fetchTimeslotsData() {
  try {
    ...
    console.log('[視野予約枠数] JSONデータ取得:', data);

    return data;

  } catch (error) {
    console.error('[視野予約枠数] データ取得エラー:', error);
    return null;
  }
}
...
async function updateBadge(slotsCount, displayText, status) {
  ...
  const title = `予約枠数（視野）: ${displayText} ${statusText}\n最終更新: ${timeStr}\n次回更新: ${secondsUntilNext}秒後`;

  chrome.action.setTitle({ title });
  console.log(`[視野予約枠数] バッジ更新: ${displayText} ${statusText}`);
...
async function updateBadgeError() {
  ...
  const title = `予約枠数（視野）: エラー\n最終更新: ${timeStr}`;
  chrome.action.setTitle({ title });
  console.log('[視野予約枠数] バッジ更新: エラー');
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'updateNow') {
    console.log('[視野予約枠数] 手動更新が要求されました');
    updateStatus().then(() => {
