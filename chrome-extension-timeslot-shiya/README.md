# 予約枠数表示（視野）Chrome拡張機能

視野予約の空き枠数をChromeタスクバーにリアルタイム表示する拡張機能です。
...
- **2本構成タイムラインバー**（午前10〜13時/午後15〜18時）
  - 午前バー: 青色（#3498db）背景
  - 午後バー: オレンジ色（#e67e22）背景
  - 白抜きドット（白い中心＋緑枠線#006633）で空き枠を可視化
...
4. このディレクトリ（`chrome-extension-timeslot-shiya`）を選択

## 📁 ファイル構成

```
chrome-extension-timeslot-shiya/
...
- **データ生成システム**: `docker-timeslot-checker-shiya/`
- **一般予約版**: `chrome-extension-timeslot-general/`
