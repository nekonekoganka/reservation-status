...
- `timeslot-checker-unified-general-offpeak`（一般予約・夜間**5分毎**）
...
- `generate-daily-summary-general`（日次集計・一般・毎日0:05）
- `generate-daily-summary-shiya`（日次集計・視野・毎日0:10）
...
- 0:00〜6:59, 17:00〜23:59（14時間）: 5分間隔 = 14 × 60 / 5 = **168回/日**
- **小計: 648回/日**
...
### 合計
- **通常日: 858回/日**（一般648 + 視野210）
- **水曜日: 378回/日**（一般234 + 視野144）
- **週平均: 約789回/日**
- **月間: 約23,700回/月**（変更前: 2,880回/日 → 約86,400回/月）
...
| 2026/2/8 | **追加最適化3**: 検討のみ、未適用（一般予約・夜間5分/視野予約・日中5分を維持） |
...
2月の請求額調査を受け、以下の変更を検討しましたが、**最終的にどちらも適用しませんでした**：
- ~~一般予約・夜間（0-6時, 17-23時）: 5分間隔 → 10分間隔~~ → **未適用（5分のまま）**
- ~~視野予約・日中（7-17時、水曜除く）: 5分間隔 → 10分間隔~~ → **未適用（5分のまま）**

現在の設定は追加最適化2の状態を維持しています。
